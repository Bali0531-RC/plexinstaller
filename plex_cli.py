...
def show_help():
    """
    Print the CLI usage guide and examples to the terminal.
    
    Displays available commands, tools (including the tool setupdomain flow), addon-management subcommands, and common usage examples for the Plex CLI management tool.
    """
...
def tool_setupdomain(app: str) -> int:
    """
    Set up a domain, Nginx reverse proxy, firewall rules, and an SSL certificate for an installed instance.
    
    Performs DNS validation, opens the required firewall port, creates an Nginx site that proxies the given domain to the instance's local port, and obtains/configures an SSL certificate for the domain. This command must be run as root and will prompt interactively for any missing information (port, domain, email).
    
    Parameters:
        app (str): The application instance name or prefix to resolve to an installed instance.
...
def handle_tool_command(args: list) -> int:
    """
    Dispatch tool-related subcommands.
    
    Currently supports the `setupdomain` subcommand to configure a domain, reverse proxy, and SSL for an application instance. When invoked with no subcommand or with an unknown subcommand, prints usage information.
    
    Parameters:
        args (list): Command-line arguments for the tool subcommand; the first element should be the subcommand name followed by its options.
    
    Returns:
        int: `0` on success, non-zero on error (e.g., usage errors or unknown subcommands).
    """
    if len(args) < 1:
...
def main():
    """
    Dispatch CLI subcommands for the Plex installer and return an exit code.
    
    Reads sys.argv, runs an automatic update check, and routes commands such as list, start, stop, restart, status, logs, config, enable, disable, debug, addon, and tool to their handlers; prints help or usage errors when invocation is invalid.
    
    Returns:
        int: Exit code where `0` indicates success and non-zero indicates an error.
    """
    _maybe_auto_update()
